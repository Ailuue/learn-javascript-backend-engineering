 ...
 *     job and returns a Job immediately (the analog of Celery's `.delay()`).
 ...
 *
 * Ways to enqueue (compare to Celery):
 *   queue.add("add", { x, y })                    → like add.delay(x, y)
 *   queue.add("add", { x, y }, { delay: 10_000 }) → like apply_async(countdown=10)
 *
 * Unlike Celery there's no "call the function directly and run it now" mode —
 * the processor is just a normal async function, so for synchronous use you call
 * that function yourself (see `add` below).
 *
...
async function main() {
  ...
  // --- Delayed job (Celery's apply_async countdown) ---
